document.addEventListener('change', function(evento){
    if(evento.target && evento.target.classList && evento.target.classList.contains('check-tarefa')){
...
if(botaoStatus){
    ...
    botaoStatus.addEventListener('click', function(){
        const atual = this.dataset.status === 'concluida' ? 'concluida' : 'pendente';
...
function criarEtiquetaStatus(status){
    ...
    etiqueta.className = 'task-status ' + (status === 'concluida' ? 'concluida' : 'pendente');
